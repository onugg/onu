...
export async function deleteAndEmitEventIfExists(emitEventCallBack: Function, discordMemberDeleteArgs: Prisma.DiscordMemberDeleteArgs) {
  ...
  if (totalMembersForUser === 0) {
    discordUser.deleteAndEmitEventIfExists(emitEventCallBack, {where: {discordId: discordMemberDeleteArgs.where.discordUserId_discordGuildId?.discordUserId!}})
  }
}

export async function createAndEmitEventIfNotExists(emitEventCallBack: Function, discordMemberCreateArgs: Prisma.DiscordMemberCreateArgs) {
  var member = await prisma.discordMember.findUnique({
    where: {
