...
async function validateEvent(sourcename: string, type: string, event: any) {
  try {
    const validSources = Array.from(brokers.brokerList.map(broker => broker.eventSourceName))
    ...
    var broker = brokers.brokerList.find(broker => broker.eventSourceName == sourcename)!

    var validEventTypes = Array.from(broker.events.map((event: Event) => event.name))

    if (!validEventTypes.includes(type)) {
      throw(`type header does not reference a valid event type. Valid event types for the source ${validSources.join(", ")} are [${validEventTypes.join(", ")}]`)
    }

    var eventVersions = broker.events.filter((event: Event) => event.name == type).map((event: Event) => event.version)
    ...
    var eventSchema = broker.events.find((event: Event) => event.name == type)!.EventSchema

    var parse = eventSchema.safeParse(event)
    if (!parse.success) {
      const formatted = parse.error.format();
      throw(`event body does not match the schema for the event type ${type}. Error: ${JSON.stringify(formatted)}`)
    }
...
export const eventEmitter = (options: {brokerUrl: string}) => {
  ...
  async function emitEvent(options: {sourcename: string, type: string, event: any}) {
    ...
    console.log(`Emitting event id ${ce.id} from source ${sourcename} with type ${type}`)
...
export const startEventReceivingWebServer = (options: httpReceiverOptions) => {
  ...
  const sendEventToCallback = (eventSource: string, eventType: string, eventPayload: string)  => {
  ...
  e.post('/', 
    ...
    function(req: Request, res: Response) {
      res.status(202).end()
      if (res.locals.error) {
        console.error(`Error handing event sent from broker: ${res.locals.error}`);
      } else {
        sendEventToCallback(res.locals.source, res.locals.eventType, res.locals.eventMessage)
      }
    }
  )
